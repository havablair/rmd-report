#External script for soil organic matter (SOM) graphs in farmer reports

## @knitr som

make_som_graph <- function(reg, farmer.name){
  ...
  #for region data, add group column and label "Regional Farms" and "Regional Natural Areas" Note: region now includes target farmer data.
  ...
  #Same for natural area samples.  This doesn't seem ideal.  
  ...
  farm.means <- indiv.data %>% 
    ...
    summarise(mean = mean(OM, na.rm = T), sd = sd(OM, na.rm = T))
  ...
  g <- ggplot() +
    ...
    geom_jitter(data = all.points, aes(x = group, y = OM, color = group), height = 0, width = 0.15, alpha = 0.5, show.legend = FALSE) +
    
    guides(x = guide_axis(angle = -40)) + 
    
    labs(title = glue("{farmer.name} Soil Organic Matter"), x = element_blank(), y = "Soil Organic Matter %", caption = str_wrap("Error bars represent 1 standard deviation from the mean. Black points and text labels are the mean of measurements in that category. Smaller colored points represent individual sample measurements.", width = 100)) + 
