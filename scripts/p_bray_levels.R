#Reads a CSV containing fertility recommendations for phosphorus (P) Bray/Mehlich III derived from UMN fertilizer guidelines
#for agronomic crops.  Min and Max functions defined below return a row index (integer) that is used to subset the full "bands"
#dataframe within the associated graphing functions (external) for the farmer report. This subsetted dataframe is used to define
#some color bands to illustrate relative nutrient levels (Low/Medium/High)

## @knitr

pb.bands <- read.csv("./raw-data/p_br_me_bands.csv", stringsAsFactors = FALSE)

colnames(pb.bands) <- c("level", "start", "end")
...
pb.min <- function(low){
  if (low > 0 & low < 5) {
    return(1) #VL
  } else if (low > 6 & low < 11){
    return(2) #L
  } else if (low > 12 & low <15){
    return(3) #M
  } else if (low > 16 & low > 20){
    return(4) #H
  } else {
    return(5) #VH
  }}
...
pb.max <- function(high){
  if (high > 0 & high < 5) {
    return(1) #VL
  } else if (high > 6 & high < 11){
    return(2) #L
  } else if (high > 12 & high <15){
    return(3) #M
  } else if (high > 16 & high > 20){
    return(4) #H
  } else {
    return(5) #VH
  }}
...
make_p_bray <- function(reg, farmer.name){
  ...
  p.bray <- full_join(p.bray, meta, by = "Sample.ID")
  
  p.bray$type <- as.factor(p.bray$type)
  
  p.bray$label <- paste(p.bray$position, p.bray$type, sep = " : ")
  
  #####DATA SUBSET######
  
  #subset the relevant regional data  
  reg.data <- subset(p.bray, region == reg)
  indiv.data <- subset(p.bray, farmer == farmer.name)
  ...
  #subset the generic P bray levels df to include only levels relevant to current dataset
  pb.bands.df <- pb.bands[pb.min(low):pb.max(high),]
  pb.bands.df[1,2] <- low - 5
  pb.bands.df[nrow(pb.bands.df),3] <- high + 5
  ...
  g <- ggplot()+
    ...
    geom_rect(data = pb.bands.df, aes(ymin = start, ymax = end, NULL, NULL, fill = level), xmin = 0, xmax = 5, alpha = 0.3) +
    ...
    labs(title = glue("{farmer.name} Phosphorus (Bray)"), x = element_blank(), y = "Phosphorus (ppm)", caption = str_wrap("Error bars represent 1 standard deviation from the mean. Black points and text labels are the mean of measurements in that category. Smaller colored points represent individual sample measurements.", width = 100)) + 
